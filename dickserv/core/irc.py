...
# https://git.supernets.org/acidvegas/dickserv
# irc.py
			...
			if chan == config.connection.channel and (DickServ.status or ident == config.settings.admin):
				if not msg.startswith(config.settings.cmd_char):
					...
					if urls:
						...
						Commands.sendmsg(chan, constants.bold + 'DickServ IRC Bot - Developed by acidvegas in Python - https://git.supernets.org/acidvegas/dickserv')
